...
#    atime: 'on'
    ...
    options:  # any options the zpool command accepts
      ...
      - '-L'
    state: 'present'
    scrub_cron:  # override global scrub cron job parameters per zpool
...
#    atime: 'on'
    ...
    type: 'mirror'  #define pool type... | basic (no-raid) | mirror | raidz | raidz2 | raidz3
    state: 'present'
zfs_pools_scrub_cron:  # defines global scrub cron job parameters. Only applies when `zfs_enable_monitoring` is set to True.
  minute: '0'
  hour: '0'
  day: '*'
  month: '*'
...
zfs_monitoring_capacity_threshold: 80  # in percentage.
zfs_monitoring_scrub_max_age: 8  # in days.
